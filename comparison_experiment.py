            ...
            for discr, discr_descr in approaches.discretization_methods.items():
                ...
                for acq, _ in approaches.acquisition_functions.items():
                    if learn == "GP":
                        ...
                        if discr == "categorical":
                            for n_features in range(config.min_nr_features, config.max_nr_features+1, config.iter_step_nr_features):
                                 mp_results.append(("bayopt", dataset_id, (pool.apply_async(__run_bay_and_add_test_score, [], {"algo": algo, "algo_descr":algo_descr, "learn_descr":learn_descr, "kernel_descr":kernel_descr, "discr_descr":discr_descr, "acq":acq, "data_training":data_training, "data_test": data_test, "target_training":target_training, "target_test":target_test, "learning_method":learn, "kernel":None, "discretization_method":discr, "n_features":n_features, "estimator":estimator, "acq_func":acq, "metric":metric, "n_calls":n_calls, "cross_validation":config.n_splits_bay_opt, "acq_optimizer":acq_optimizer, "n_convergence":config.n_convergence, "n_acq_points":config.n_acq_points}))))
